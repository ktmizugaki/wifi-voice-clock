...
static const char PASSWORD_CHARS[] = {
    ...
    0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x9C, 0x4F, 0x50, 0x51,
    0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A,
    /* a-z */
    0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69,
    0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x9B, 0x6F, 0x70, 0x71,
...
esp_err_t simple_wifi_get_ssid(char ssid[SWIFI_SSID_LEN])
{
    ...
    if (s_ssid[0] == 0) {
        ...
        snprintf(s_ssid, sizeof(s_ssid),
            "esp32-%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    }
    strcpy(ssid, s_ssid);
    return ESP_OK;
}
...
esp_err_t simple_wifi_get_password(char password[SWIFI_PW_LEN])
{
    ...
    err = nvs_open("ota_softap", NVS_READWRITE, &nvsh);
    if (err != ESP_OK) {
        return err;
    }
